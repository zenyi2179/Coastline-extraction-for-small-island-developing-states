...
def select_cou_poly(valid_pixel_after_shp, GID_shp, cou_valid_poly):
    ...
    select_layer = arcpy.management.SelectLayerByLocation(
        in_layer=[valid_pixel_after_shp], overlap_type="INTERSECT", select_features=GID_shp,
...
def sel_val_cou_map(year, country, z_value=5):
    ...
    for record in records_list:
        try:
            ...
            vector_output_path = fr'E:\_OrderingProject\F_IslandsBoundaryChange\b_ArcData\temp\filterValidPixel\{sids_cou}_{map_name}_valid_pixel.shp'
            filter_valid_pixel_to_shp(input_tif_path, vector_output_path, threshold=z_value)
...
def mer_val_cou_map(GID_shp_list, merge_output):
    ...
    arcpy.analysis.Buffer(in_features=merge_layer, out_feature_class=fr"in_memory\gid_buffer",
                          buffer_distance_or_field="15 Meters", method="GEODESIC")

    # 步骤 2：对合并后的图层进行成对融合（使用 Pairwise Dissolve 工具）
    temp_merge_output = fr"in_memory\mvcm_temp_merge"  # 临时存储融合后的结果
    arcpy.analysis.PairwiseDissolve(
        in_features=fr"in_memory\gid_buffer",
        out_feature_class=temp_merge_output,
        multi_part="MULTI_PART"  # 输出多个部分，而不是单一的几何部分
    )

    arcpy.analysis.Buffer(in_features=temp_merge_output, out_feature_class=fr"in_memory\gid_buffer_2",
                          buffer_distance_or_field="-15 Meters", method="GEODESIC")

    # 步骤 3：对融合后的面进行平滑处理（使用 SmoothPolygon 工具）
    temp_polygon = fr"in_memory\mvcm_temp_polygon"
    arcpy.management.FeatureToPolygon(in_features=[fr"in_memory\gid_buffer_2"],
    ...
    arcpy.cartography.SmoothPolygon(
        ...
        tolerance="30 Meters",  # 平滑的容忍度为 50 米
